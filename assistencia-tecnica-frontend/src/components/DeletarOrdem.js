...
function DeletarOrdem({ ordemId, onDeleteSuccess }) {
    const handleDelete = async () => {
        try {
            ...
            if (response.status === 204 || response.status === 200) {
                // Se a deleção foi bem-sucedida, chama o callback de sucesso
                onDeleteSuccess(ordemId);  
            } else {
                const textResult = await response.text();
                console.error('Erro ao deletar ordem:', textResult);
